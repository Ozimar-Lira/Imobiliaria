...
const database_name = 'Imoveis.db'; //Nome do banco de dados
      ...
      SQLite.echoTest()
        .then(() => {
          SQLite.openDatabase(
            ...
            .then(DB => {
              db = DB;
              db.executeSql('SELECT 1 FROM Imoveis LIMIT 1')
                .then(() => {
                  console.log(
                ...
                .catch(error => {
                  ...
                  db.transaction(tx => {
                    tx.executeSql(
                      'CREATE TABLE IF NOT EXISTS Imoveis (id INTEGER PRIMARY KEY AUTOINCREMENT, endereco VARCHAR(100), finalidade VARCHAR(30), tipo VARCHAR(30), valor INTEGER, imagem TEXT);',
                    );
                  })
                    .then(() => {
      ...
      this.Conectar()
        .then(db => {
          db.transaction(tx => {
            ...
            tx.executeSql(
              'INSERT INTO Imoveis (endereco, finalidade, tipo, valor, imagem) VALUES (?, ?, ?, ?, ?)',
              [
                item.endereco,
                item.finalidade,
                item.tipo,
  ...
  Remover(id) {
    return new Promise(resolve => {
      this.Conectar()
        .then(db => {
          db.transaction(tx => {
            tx.executeSql('DELETE FROM Imoveis WHERE id = ?', [id]).then(
              ([tx, results]) => {
                console.log(results);
                resolve(results);
              },
